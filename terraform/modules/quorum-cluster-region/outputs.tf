...
output "quorum_maker_node_dns" {
  value = "${data.aws_instances.quorum_maker_node.*.public_dns}"
}

output "quorum_validator_node_dns" {
  value = "${data.aws_instances.quorum_validator_node.*.public_dns}"
}

output "quorum_observer_node_ips" {
  value = "${coalescelist(aws_eip.quorum_observer.*.public_ip, data.aws_instances.quorum_observer_node.*.public_ip)}"
}
