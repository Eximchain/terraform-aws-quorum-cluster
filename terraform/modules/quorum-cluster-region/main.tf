...
data "template_file" "quorum_public_cidr_block"  {
  template = "$${cidr_block}"
...
data "template_file" "quorum_private_cidr_block"  {
  template = "$${cidr_block}"

  vars {
    cidr_block = "${cidrsubnet(data.template_file.quorum_cidr_block.rendered, 3, 6)}"
  }
}
