...
resource "aws_nat_gateway" "backup_lambda" {
  ...
  subnet_id     = "${lookup(var.validator_node_counts, var.aws_region, 0)>0 ? aws_subnet.quorum_validator.id : 
  lookup(var.maker_node_counts, var.aws_region, 0)>0 ? aws_subnet.quorum_maker.id : lookup(var.observer_node_counts, var.aws_region, 0)>0 ? aws_subnet.quorum_observer.id : aws_subnet.backup_lambda.id}"
