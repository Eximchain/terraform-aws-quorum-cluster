...
function emit_rpc_metric {
  ...
  local RESPONSE_JSON=$(curl -X POST -H "Content-Type: application/json" --data "{\"jsonrpc\":\"2.0\",\"method\":\"$METHOD\",\"params\":[$PARAMS],\"id\":1}" $RPC_ADDR:$RPC_PORT)
...
function emit_peer_count_metrics {
  local RESPONSE_JSON=$(curl -X POST -H "Content-Type: application/json" --data '{"jsonrpc":"2.0","method":"net_peerCount","params":[],"id":1}' $RPC_ADDR:$RPC_PORT)
...
function emit_block_skew_metrics {
  ...
  local readonly RESPONSE_JSON_LOCAL=$(curl -X POST -H "Content-Type: application/json" --data '{"jsonrpc":"2.0","method":"eth_blockNumber","params":[],"id":1}' $RPC_ADDR:$RPC_PORT)
